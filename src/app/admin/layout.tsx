...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const usingDemoPassword = verifyPassword("shaibal123", admin.passwordHash);
  return (
    <div className="admin pt-16 min-h-screen lg:grid lg:grid-cols-[250px_1fr]">
      ...
      <div className="min-w-0">
        ...
        {usingDemoPassword && (
          <div className="mx-4 sm:mx-6 lg:mx-8 mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm dark:border-amber-500/30 dark:bg-amber-500/10" role="alert">
            <span className="font-semibold">Security notice:</span>
            <span className="text-muted">this admin account still uses the demo password.</span>
            <Link href="/account?tab=profile#password" className="btn btn-primary btn-sm ml-auto">Change password</Link>
          </div>
        )}
