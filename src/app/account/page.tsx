...
import { ProfileForm, PasswordForm, LogoutButton, MarkReadButton } from "@/components/account-widgets";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const usingDemoPassword = verifyPassword("shaibal123", user.passwordHash);
        ...
        <div className="space-y-6 min-w-0">
          ...
          {tab === "profile" && (
            <>
              ...
              <div className="card p-6" id="password">
                <h2 className="font-display font-bold text-lg mb-1">Password</h2>
                {usingDemoPassword ? (
                  <p className="text-sm mb-4 rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-500/10 dark:border-amber-500/30 px-3 py-2.5">You are still using the demo password. Set a new one before sharing this site publicly.</p>
                ) : (
                  <p className="text-sm text-muted mb-4">Use at least 8 characters. Changing it signs out all other devices.</p>
                )}
                <PasswordForm />
              </div>
            </>
